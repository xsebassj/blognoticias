...
class PostDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["liked_by_user"] = post.like_set.filter(autor=user).exists()
        context["likes_count"] = post.like_set.count()
        context['comment_form'] = CommentForm()
        context['active_images'] = self.object.images.filter(active=True)
        context['has_images'] = context['active_images'].exists()
        ...
        if delete_comment_id:
            ...
            if (
                ...
                or (
                    comment.post.autor == self.request.user
...
class CommentCreateView(CreateView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "noticias:post_detail", kwargs={"slug": self.object.post.slug}
        )
...
class CategoryListView(ListView):
    model = Category
    template_name = 'noticias/post_list'
